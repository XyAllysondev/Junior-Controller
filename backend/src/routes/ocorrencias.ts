...
import { z } from 'zod';
import { AGORA_SQL, agora, rodar, todos, toSqlDateTime, turnoDoHorario, um } from '../db.js';
...
const buscarPorId = (id: number | string) => um(`${SELECT_BASE} WHERE o.id = ?`, [id]);
...
ocorrenciasRouter.get(
  ...
  rota(async (req, res) => {
    ...
    const contagem = await um<{ n: number }>(
    ...
    const itens = await todos(
      `${SELECT_BASE} ${filtro}
       ...
       LIMIT ? OFFSET ?`,
      [...params, porPagina, (pagina - 1) * porPagina] as any,
    );
...
ocorrenciasRouter.post(
  ...
  rota(async (req, res) => {
    ...
    const maquina = await um('SELECT id FROM maquinas WHERE id = ?', [d.maquina_id]);
    ...
    const info = await rodar(
      ...
      [
        ...
        d.motivo_id ?? null,
        d.tecnico_id ?? null,
        d.turno_id ?? (await turnoDoHorario(aberto)),
        d.tipo,
        d.prioridade,
        statusDerivado(d),
        d.descricao,
    ...
    res.status(201).json(await buscarPorId(info.id));
  }),
);
...
ocorrenciasRouter.put(
  '/:id',
  rota(async (req, res) => {
    const atual = await um<Record<string, any>>('SELECT * FROM ocorrencias WHERE id = ?', [
      req.params.id,
    ]);
    ...
    if (d.aberto_em && d.turno_id === undefined) {
      d.turno_id = await turnoDoHorario(d.aberto_em);
    }
    ...
    if (colunas.length) {
      await rodar(
        `UPDATE ocorrencias SET ${colunas.map((c) => `${c} = ?`).join(', ')},
          atualizado_em = ${AGORA_SQL} WHERE id = ?`,
        [...colunas.map((c) => d[c] ?? null), req.params.id],
      );
    }
    ...
    await rodar(
      `UPDATE ocorrencias
         SET atendido_em = ?, tecnico_id = ?, status = 'Em atendimento',
             atualizado_em = ${AGORA_SQL}
       WHERE id = ?`,
      [quando, tecnico_id ?? null, req.params.id],
    );
...
ocorrenciasRouter.post(
  ...
  rota(async (req, res) => {
    ...
    // Se ninguem marcou o inicio do atendimento, usamos a abertura: assumir
    // que comecou junto com o fim inventaria um TA falso de zero.
    ...
    await rodar(
      `UPDATE ocorrencias
         SET fim_em = ?, atendido_em = ?, tecnico_id = ?, solucao = COALESCE(?, solucao),
             status = 'Concluida', atualizado_em = ${AGORA_SQL}
       WHERE id = ?`,
      [fim, atendido, tecnico_id ?? null, req.body?.solucao?.trim() || null, req.params.id],
    );
...
ocorrenciasRouter.post(
  '/:id/reabrir',
  rota(async (req, res) => {
    const atual = await um('SELECT id FROM ocorrencias WHERE id = ?', [req.params.id]);
    if (!atual) return res.status(404).json({ erro: 'Ocorrencia nao encontrada' });

    await rodar(
      `UPDATE ocorrencias
         SET fim_em = NULL,
             status = CASE WHEN atendido_em IS NULL THEN 'Aberta' ELSE 'Em atendimento' END,
             atualizado_em = ${AGORA_SQL}
       WHERE id = ?`,
      [req.params.id],
    );
...
ocorrenciasRouter.delete(
  '/:id',
  rota(async (req, res) => {
    const info = await rodar('DELETE FROM ocorrencias WHERE id = ?', [req.params.id]);
    if (info.alteradas === 0) return res.status(404).json({ erro: 'Ocorrencia nao encontrada' });
